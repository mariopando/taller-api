...
export class MercadoPagoService {
  ...
  async createPreference(
    ...
    try {
      ...
      this.logger.debug(`Mercado Pago preference created: ${preferenceId} for amount ${amount} ${currency}`)
      ...
      this.logger.error(`Mercado Pago preference creation failed: ${error.message}`)
      throw new HttpException(`Mercado Pago preference creation failed: ${error.message}`, HttpStatus.PAYMENT_REQUIRED)
    }
  }

  async getPaymentInfo(
    paymentId: string,
  ): Promise<{ status: PaymentStatus; statusDetail: string; amount?: number; authCode?: string }> {
    try {
      ...
      this.logger.error(`Mercado Pago payment info retrieval failed: ${error.message}`)
  ...
  async refundPayment(
    paymentId: string,
    amount?: number,
  ): Promise<{ status: PaymentStatus; refundAmount: number; refundId: string }> {
    try {
      ...
      this.logger.debug(`Mercado Pago refund processed: ${paymentId} for amount ${amount}`)
      return result
    } catch (error) {
      this.logger.error(`Mercado Pago refund failed: ${error.message}`)
      throw new HttpException(`Mercado Pago refund failed: ${error.message}`, HttpStatus.BAD_REQUEST)
    }
  }

  async validateNotification(notificationData: Record<string, any>): Promise<boolean> {
    try {
      this.logger.debug("Mercado Pago notification validated")
      return true
    } catch (error) {
      this.logger.error(`Mercado Pago notification validation failed: ${error.message}`)
      return false
    }
  }
