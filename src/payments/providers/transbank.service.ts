...
export class TransbankService {
  ...
  async initializeTransaction(
    ...
    try {
      ...
      this.logger.debug(`Transbank transaction initialized: ${transactionId} for amount ${amount}`)
      ...
      throw new HttpException(`Transbank initialization failed: ${error.message}`, HttpStatus.PAYMENT_REQUIRED)
    }
  }

  async confirmTransaction(
    transactionToken: string,
  ): Promise<{ status: PaymentStatus; authCode?: string; amount?: number; responseCode?: number }> {
    try {
      ...
      throw new HttpException(`Transbank confirmation failed: ${error.message}`, HttpStatus.PAYMENT_REQUIRED)
    }
  }
  ...
  async refundTransaction(
    ...
    try {
      ...
      this.logger.debug(`Transbank refund processed: ${transactionToken} for amount ${amount}`)
      return result
    } catch (error) {
      this.logger.error(`Transbank refund failed: ${error.message}`)
      throw new HttpException(`Transbank refund failed: ${error.message}`, HttpStatus.BAD_REQUEST)
    }
  }

  async getTransactionStatus(
    transactionToken: string,
  ): Promise<{ status: PaymentStatus; responseCode: number; authCode?: string }> {
    try {
      const result = {
        status: PaymentStatus.CAPTURED,
        responseCode: 0,
      ...
      throw new HttpException(`Transbank status check failed: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }
