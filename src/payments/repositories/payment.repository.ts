...
import { PaymentStatus, type PaymentProvider } from "src/common/enums/payment-provider.enum"
...
export class PaymentRepository extends Repository<Payment> {
  ...
  async findByTransactionId(transactionId: string, provider: PaymentProvider): Promise<Payment | null> {
    return this.findOne({
  ...
  async findByProviderAndStatus(provider: PaymentProvider, status: PaymentStatus, limit = 10): Promise<Payment[]> {
    return this.find({
