...
import type { ConfirmPaymentRequestDto, RefundPaymentRequestDto } from "@/common/dto/payment-request.dto"
import type { PaymentResponseDto, PaymentCallbackDto } from "@/common/dto/payment-response.dto"
import { PaymentStatus, PaymentProvider } from "@/common/enums/payment-provider.enum"
...
export class PaymentsController {
  ...
  @Post("confirm")
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: "Confirm payment after customer completes the transaction" })
  @ApiResponse({ status: 200, description: "Payment confirmed successfully" })
  @ApiResponse({ status: 404, description: "Payment not found" })
  @ApiResponse({ status: 402, description: "Payment provider error" })
  async confirmPayment(confirmPaymentDto: ConfirmPaymentRequestDto): Promise<PaymentResponseDto> {
  ...
  @ApiQuery({ name: "provider", enum: PaymentProvider, description: "Payment provider" })
  ...
  async refundPayment(transactionId: string, refundPaymentDto: RefundPaymentRequestDto): Promise<PaymentResponseDto> {
    return this.paymentsService.refundPayment(transactionId, refundPaymentDto.provider, refundPaymentDto.amount)
  }
  ...
  async listPayments(status?: PaymentStatus, limit?: number): Promise<PaymentResponseDto[]> {
