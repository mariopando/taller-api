...
describe("PaymentsService", () => {
  ...
  beforeEach(async () => {
    ...
    transbankService = module.get(TransbankService) as jest.Mocked<TransbankService>
    mercadoPagoService = module.get(MercadoPagoService) as jest.Mocked<MercadoPagoService>
  })
  ...
  describe("confirmPayment", () => {
    it("should confirm a Transbank payment", async () => {
      ...
      const result = await service.confirmPayment(transactionId, token, provider)
    ...
    it("should throw NotFoundException when payment does not exist", async () => {
      ...
      await expect(service.confirmPayment("INVALID_TXN", "TOKEN", PaymentProvider.TRANSBANK)).rejects.toThrow(
        NotFoundException,
      )
    })
  })
  ...
  describe("getPaymentStatus", () => {
    it("should return payment status", async () => {
      ...
      const result = await service.getPaymentStatus("TXN_123", PaymentProvider.TRANSBANK)
  ...
  describe("refundPayment", () => {
    it("should refund a captured payment", async () => {
      ...
      const result = await service.refundPayment("TXN_123", PaymentProvider.TRANSBANK)
    ...
    it("should throw BadRequestException when trying to refund non-captured payment", async () => {
      ...
      await expect(service.refundPayment("TXN_123", PaymentProvider.TRANSBANK)).rejects.toThrow(BadRequestException)
    })
  })
  ...
  describe("listPayments", () => {
    ...
    it("should list payments filtered by status", async () => {
      ...
      expect(paymentRepository.findByStatus).toHaveBeenCalledWith(PaymentStatus.PENDING, 10)
    })
  })
})
