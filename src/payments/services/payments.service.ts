...
import { PaymentStatus, PaymentProvider } from "src/common/enums/payment-provider.enum"
...
export class PaymentsService {
  ...
  async createPayment(createPaymentDto: CreatePaymentDto): Promise<PaymentInitializationDto> {
  ...
  async confirmPayment(transactionId: string, token: string, provider: PaymentProvider): Promise<PaymentResponseDto> {
    const payment = await this.paymentRepository.findByTransactionId(transactionId, provider)
    ...
    try {
      ...
      await this.paymentRepository.updatePaymentStatus(payment.id, PaymentStatus.ERROR, null, error.message)
      throw error
    }
  }

  async getPaymentStatus(transactionId: string, provider: PaymentProvider): Promise<PaymentResponseDto> {
    const payment = await this.paymentRepository.findByTransactionId(transactionId, provider)
  ...
  async refundPayment(transactionId: string, provider: PaymentProvider, amount?: number): Promise<PaymentResponseDto> {
    const payment = await this.paymentRepository.findByTransactionId(transactionId, provider)
    ...
    try {
      ...
      if (provider === PaymentProvider.TRANSBANK) {
        result = await this.transbankService.refundTransaction(payment.transactionId, amount || payment.amount)
      } else if (provider === PaymentProvider.MERCADO_PAGO) {
        result = await this.mercadoPagoService.refundPayment(payment.transactionId, amount || payment.amount)
      }
  ...
  async handleCallback(callbackData: PaymentCallbackDto): Promise<Payment> {
    const payment = await this.paymentRepository.findByTransactionId(callbackData.transactionId, callbackData.provider)
  ...
  async listPayments(status?: PaymentStatus, limit = 10): Promise<PaymentResponseDto[]> {
